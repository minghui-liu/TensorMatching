...
texture<double, 2, cudaReadModeElementType> texA;
...
/**
  ...
  */
__global__ void cuComputeDistanceTexture(int wA, double * B, int wB, int pB, int dim, double* AB){
...
/**
  ...
  * @param AB    pointer on the matrix containing the wA*wB distances computed
  */
__global__ void cuComputeDistanceGlobal( double* A, int wA, int pA, double* B, int wB, int pB, int dim,  double* AB){

	// Declaration of the shared memory arrays As and Bs used to store the sub-matrix of A and B
	__shared__ double shared_A[BLOCK_DIM][BLOCK_DIM];
	__shared__ double shared_B[BLOCK_DIM][BLOCK_DIM];
...
/**
  ...
  * @param k       number of neighbors to consider
  */
__global__ void cuParallelSqrt(double *dist, int width, int pitch, int k){
    unsigned int xIndex = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int yIndex = blockIdx.y * blockDim.y + threadIdx.y;
...
void knn(double* ref_host, int ref_width, double* query_host, int query_width, int height, int k, double* dist_host, int* ind_host){
    ...
    unsigned int use_texture = ( ref_width*size_of_double<=MAX_TEXTURE_WIDTH_IN_BYTES && height*size_of_double<=MAX_TEXTURE_HEIGHT_IN_BYTES );
    ...
    max_nb_query_traited = ( memory_free * MAX_PART_OF_FREE_MEMORY_USED - size_of_double * ref_width*height ) / ( size_of_double * (height + ref_width) + size_of_int * k);
    ...
    result = cudaMallocPitch( (void **) &query_dev, &query_pitch_in_bytes, max_nb_query_traited * size_of_double, height + ref_width);
    if (result){
    ...
    if (use_texture){
        ...
        cudaChannelFormatDesc channelDescA = cudaCreateChannelDesc<double>();
        result = cudaMallocArray( &ref_array, &channelDescA, ref_width, height );
        if (result){
            printErrorMessage(result, ref_width*height*size_of_double);
            cudaFree(ind_dev);
            cudaFree(query_dev);
            return;
        }
        cudaMemcpyToArray( ref_array, 0, 0, ref_host, ref_width * height * size_of_double, cudaMemcpyHostToDevice );
        ...
        result = cudaMallocPitch( (void **) &ref_dev, &ref_pitch_in_bytes, ref_width * size_of_double, height);
        if (result){
            printErrorMessage(result,  ref_width*size_of_double*height);
            cudaFree(ind_dev);
            cudaFree(query_dev);
            return;
        }
        ref_pitch = ref_pitch_in_bytes/size_of_double;
        cudaMemcpy2D(ref_dev, ref_pitch_in_bytes, ref_host, ref_width*size_of_double,  ref_width*size_of_double, height, cudaMemcpyHostToDevice);
    }
...
int main(void){
	...
	ind    = (int *)   malloc(query_nb * k * sizeof(double));
