...
void knn(double* ref_host, int ref_width, double* query_host, int query_width, int height, int k, double* dist_host, int* ind_host) {
    ...
    for (int i=0; i<query_width; i+=max_nb_query_traited){
        ...
        double ntwo = -2.0;
        double zero = 0.0;
        cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_T, (int)query_pitch, (int)ref_pitch, height, &ntwo, query_dev, query_pitch, ref_dev, ref_pitch, &zero, dist_dev, query_pitch);
